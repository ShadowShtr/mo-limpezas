...
export function VisitOutcomeSheet({ visita, onClose, onDone }: Props) {
  const { toast } = useToast();
  const [pending, startTransition] = useTransition();

  const estadoInicial: VisitStatus = isVisitStatus(visita.status) && visita.status !== "agendada"
    ? visita.status
    : "realizada";

  const [status, setStatus] = useState<VisitStatus>(estadoInicial);
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <fieldset>
            ...
            <div className="mt-2 space-y-1.5">
              {VISIT_STATUSES.filter((s) => s !== "agendada").map((s) => (
