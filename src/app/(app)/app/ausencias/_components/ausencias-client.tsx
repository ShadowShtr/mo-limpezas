...
import { Plus, X, Loader2, Palmtree, Stethoscope, CalendarDays, AlertCircle } from "lucide-react";
import { createVacationRequest, createOwnAbsence, type VacationRequest } from "@/app/actions/vacation";
...
export function AusenciasClient({ vacations, absences }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [kind, setKind] = useState<"ferias" | "falta">("ferias");
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = kind === "ferias"
        ? await createVacationRequest({ starts_on: start, ends_on: end, notes: notes || undefined })
        : await createOwnAbsence({ absence_type: absType, starts_on: start, ends_on: end, notes: notes || undefined });
      ...
      <div>
        ...
        {vacations.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {vacations.map((v) => (
              <div key={v.id} className="bg-white rounded-2xl border border-[var(--color-border)] p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-[var(--color-text-main)]">
                      {fmt(v.starts_on)} – {fmt(v.ends_on)}
                    </p>
                    {v.days_count != null && (
                      <p className="text-xs text-[var(--color-text-muted)] mt-0.5">{v.days_count} dia(s) úteis</p>
                    )}
                  </div>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_STYLE[v.status]}`}>
                    {STATUS_LABEL[v.status]}
                  </span>
                </div>
                {v.status === "rejeitado" && v.rejection_reason && (
                  <p className="text-xs text-red-600 mt-2 bg-red-50 rounded-lg px-2 py-1.5">Motivo: {v.rejection_reason}</p>
                )}
                {v.notes && <p className="text-xs text-[var(--color-text-sub)] mt-1.5">{v.notes}</p>}
              </div>
            ))}
          </div>
        )}
        ...
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-3xl shadow-xl max-h-[90vh] overflow-y-auto safe-area-pb">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)] sticky top-0 bg-white">
              <h2 className="text-base font-semibold text-[var(--color-text-main)]">Novo pedido</h2>
            ...
            <form onSubmit={submit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setKind("ferias")}
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-colors ${kind === "ferias" ? "bg-[var(--color-primary)] text-white border-[var(--color-primary)]" : "border-[var(--color-border)] text-[var(--color-text-sub)]"}`}>
                  Férias
                </button>
                <button type="button" onClick={() => setKind("falta")}
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-colors ${kind === "falta" ? "bg-[var(--color-primary)] text-white border-[var(--color-primary)]" : "border-[var(--color-border)] text-[var(--color-text-sub)]"}`}>
                  Falta
                </button>
              </div>
              ...
              <button type="submit" disabled={pending}
                ...
                {kind === "ferias" ? "Pedir férias" : "Registar falta"}
              </button>
            </form>
          </div>
        </>
