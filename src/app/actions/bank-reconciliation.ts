...
export async function confirmMatch(matchId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  //
  //    Não é preferência de estilo: é a segunda metade de uma correcção de
  //    concorrência. As mutações manuais de um movimento trancam a linha com
  //    `FOR UPDATE` antes de decidirem se ele é editável — mas um lock só
  //    serializa contra quem o pede. Enquanto esta confirmação escrevesse sem
  //    trancar nada, alguém podia alterar ou apagar o movimento no intervalo
  //    entre a guarda e a escrita, e a conciliação ficava a afirmar uma coisa
  //    que já não era verdade.
  //
