...
export async function arquivarEquipa(
  ...
  try {
    ...
    if (!resolucao.ok) return { ok: false, error: resolucao.error };
    const { actor } = resolucao;
    if (actor.companyId !== input.companyId) return { ok: false, error: "Empresa inválida." };
    ...
    if (error) {
      if (error.message.includes(CONFLITO_EQUIPA) || error.message.includes("TEAM_ARCHIVE_CONFLICT")) {
        return {
          ok: false,
          conflito: true,
          error: "Esta equipa foi alterada por outra pessoa. Atualize para rever antes de arquivar.",
        };
      }
      return { ok: false, error: error.message };
    }
    ...
    await auditLog({
      ...
      before: { active: true, revision: input.expectedRevision },
