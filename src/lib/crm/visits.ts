...
/**
 * Os desfechos — os únicos destinos que fechar uma visita pode ter.
 *
 * 🔴 `agendada` NÃO está aqui, e a ausência é o ponto. O estado inicial é o
 *    único mutável; fechar é uma transição só de ida. Sem esta lista, o
 *    formulário de desfecho aceitava `agendada` como destino e uma visita já
 *    realizada podia ser reaberta sem rasto nenhum.
 *
 *    Corrigir um desfecho histórico é outro problema: precisa de fluxo
 *    próprio e auditado, e não se resolve alargando esta lista.
 */
export const VISIT_CLOSING_STATUSES = ["realizada", "nao_compareceu", "cancelada"] as const;

export type VisitClosingStatus = (typeof VISIT_CLOSING_STATUSES)[number];

/** O estado a partir do qual uma visita ainda pode mudar. */
export const VISIT_OPEN_STATUS = "agendada" as const;
