...
// ---------------------------------------------------------------------------
describe("o que a 101c NÃO faz", () => {
  /**
   * 🔴 Uma migration de autorização não tem nada que mexer em dados.
   *
   *    É fácil de dizer e fácil de deixar de ser verdade: bastava alguém
   *    acrescentar um «e já agora marca estes como inativos». Por isso mede-se,
   *    em vez de se afirmar.
   */
  const retratoDeDados = async () => {
    const { rows } = await pool.query(
      "SELECT id, status, auth_user_id, role, company_id FROM public.profiles ORDER BY id",
    );
    return rows;
  };

  it("não altera um único perfil nem um único estado", LENTO, async () => {
    await montarPalco(false);
    const antes = await retratoDeDados();

    await pool.query(lerSql(MIGRATION_101C));

    expect(await retratoDeDados()).toEqual(antes);
  });

  it("não escreve dados de negócio — nem uma instrução", LENTO, async () => {
    // Guarda de texto sobre o SQL executável. O `status = 'ativo'` da 101c é
    // uma CONDIÇÃO dentro de um SELECT, não uma escrita.
    const executavel = lerSql(MIGRATION_101C)
      .split(/\r?\n/)
      .filter((l) => !l.trimStart().startsWith("--"))
      .join("\n");

    expect(executavel).not.toMatch(/\bINSERT\s+INTO\b/i);
    expect(executavel).not.toMatch(/\bUPDATE\s+\w/i);
    expect(executavel).not.toMatch(/\bDELETE\s+FROM\b/i);
    expect(executavel).not.toMatch(/\bTRUNCATE\b/i);
  });

  it("não toca em contas do Auth — nem podia, mas fica dito", LENTO, async () => {
    // Banir é uma operação do Auth, feita pela aplicação. Uma migration SQL não
    // lhe chega, e não deve parecer que chega.
    const executavel = lerSql(MIGRATION_101C);
    expect(executavel).not.toMatch(/auth\.users\s+SET/i);
    expect(executavel).not.toMatch(/banned_until/i);
  });

  it("🔴 o rollback repõe EXACTAMENTE o estado pós-101b", LENTO, async () => {
    // A prova anterior mostrava que o buraco volta. Isso é necessário e não é
    // suficiente: «volta a deixar entrar» pode ser verdade com uma função
    // diferente da que lá estava. O que se exige é a definição igual.
    await montarPalco(false);

    const definicoes = async () => {
      const { rows } = await pool.query(`
        SELECT p.proname AS nome, pg_get_functiondef(p.oid) AS def,
         ...
         WHERE n.nspname='public'
           AND p.proname IN ('get_my_profile_id','can_access_service')
         ORDER BY p.proname`);
      return rows;
    };

    const posto101b = await definicoes();

    await pool.query(lerSql(MIGRATION_101C));
    expect(await definicoes()).not.toEqual(posto101b);

    await pool.query(lerSql(ROLLBACK_101C));
    expect(await definicoes()).toEqual(posto101b);
  });
});
