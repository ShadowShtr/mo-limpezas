...
const SEED = `
  ...
  VALUES('086_manual_charges_and_atomic_billing.sql','test-086');
...
async function arquivarEquipa(teamId = T1, expectedRevision?: number, expectedMembershipSnapshot?: string) {
  const revision = expectedRevision ?? (await pool.query(
    "SELECT revision FROM public.teams WHERE id=$1", [teamId])).rows[0].revision;
  const members = expectedMembershipSnapshot ?? await membershipSnapshot();
  return pool.query(
    "SELECT * FROM public.archive_team_atomic($1,$2,$3,$4,$5)",
    [EMP, GESTOR, teamId, revision, members]);
}
...
describe.sequential("🔴 Equipas R4 — arquivar, não apagar", () => {
  ...
  it("🔴 archive stale recusa por revision/snapshot e não escreve metade", async () => {
    const g1 = await pool.connect();
    const g2 = await pool.connect();
    try {
      const r0 = (await pool.query("SELECT revision FROM public.teams WHERE id=$1", [T1])).rows[0].revision;
      ...
      await g1.query(
        "SELECT * FROM public.save_permanent_team_atomic($1,$2,$3,$4,$5::uuid[],$6,$7,$8,$9,$10,$11::uuid[])",
        [EMP, GESTOR, T1, r0, [A, B].sort(), ms0, "Equipa 1", "#16A34A", true, null, [A]]);

      await expect(g2.query(
        "SELECT * FROM public.archive_team_atomic($1,$2,$3,$4,$5)",
        [EMP, GESTOR, T1, r0, ms0],
      )).rejects.toThrow(/TEAM_ARCHIVE_CONFLICT/);
...
describe.sequential("Equipas R4 — precondições fail-closed", () => {
  it("🔴 sem ledger da 086: recusa antes de qualquer DDL", async () => {
    await pool.query(BASELINE);
    await expect(pool.query(MIGRATION))
      .rejects.toThrow(/EQUIPAS_R4_MISSING_086_LEDGER/);
    const still = await pool.query(
      "SELECT count(*)::int n FROM information_schema.columns WHERE table_schema='public' AND table_name='teams' AND column_name='revision'");
...
describe.sequential("🔴 Equipas R4 — guardas adicionais pedidas pela direção", () => {
  ...
  it("🔴 viatura desativada durante a janela TOCTOU é revalidada pós-lock", async () => {
    ...
    try {
      ...
      await blocker.query("SELECT public._team_day_lock_exclusive($1,$2)", [EMP, DIA]);

      const save = daily.query(
        "SELECT * FROM public.save_team_day_allocations_atomic($1,$2,$3,$4,'[]'::jsonb,$5::jsonb)",
        [EMP, DIA, GESTOR, s0, JSON.stringify([{ team_id: T1, vehicle_id: V1, driver_id: null }])],
      ).then(() => "escreveu").catch((e: Error) => e.message);

      await esperarBloqueio(dailyPid, blockerPid);
      await pool.query("UPDATE public.vehicles SET status='inativo' WHERE id=$1", [V1]);
      await blocker.query("COMMIT");

      expect(await save).toMatch(/TEAM_ALLOCATION_INVALID_VEHICLE_PAYLOAD/);
      expect((await pool.query("SELECT status FROM public.vehicles WHERE id=$1", [V1])).rows[0].status).toBe("inativo");
      expect((await pool.query(
        "SELECT count(*)::int n FROM public.vehicle_allocations WHERE company_id=$1 AND date=$2",
        [EMP, DIA])).rows[0].n).toBe(0);
      expect((await pool.query(
        "SELECT count(*)::int n FROM public.collaborator_ride_assignments WHERE company_id=$1 AND date=$2",
        [EMP, DIA])).rows[0].n).toBe(0);
    } finally {
      try { await blocker.query("ROLLBACK"); } catch { /* já terminou */ }
      try { await daily.query("ROLLBACK"); } catch { /* já terminou */ }
      blocker.release();
      daily.release();
    }
  ...
  it("archive bloqueia serviço futuro e preserva serviço passado", async () => {
    ...
    await pool.query(
      ...
      [EMP, T1]);
    await expect(arquivarEquipa())
      .rejects.toThrow(/TEAM_ARCHIVE_BLOCKED_BY_FUTURE_ASSIGNMENTS/);
    expect((await pool.query("SELECT active FROM public.teams WHERE id=$1", [T1])).rows[0].active).toBe(true);
  });

  it("🔴 service_role-equivalente não-superuser atravessa helpers internos", async () => {
    await pool.query(`
      DO $p$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='app_service_acl') THEN
          CREATE ROLE app_service_acl LOGIN INHERIT NOSUPERUSER;
        END IF;
      END $p$;
      GRANT service_role TO app_service_acl;
      GRANT USAGE ON SCHEMA public TO service_role;
      GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO service_role;
    `);

    const app = new pg.Client({ host: "127.0.0.1", port, user: "app_service_acl", database: "eq" });
    await app.connect();
    try {
      await app.query(
        "SELECT * FROM public.save_team_day_allocations_atomic($1,$2,$3,$4,$5::jsonb,'[]'::jsonb)",
        [EMP, DIA, GESTOR, await snapshot(), JSON.stringify([{ collaborator_id: A, team_id: T2 }])]);
      const r0 = (await pool.query("SELECT revision FROM public.teams WHERE id=$1", [T1])).rows[0].revision;
      await app.query(
        "SELECT * FROM public.save_permanent_team_atomic($1,$2,$3,$4,$5::uuid[],$6,$7,$8,$9,$10,$11::uuid[])",
        [EMP, GESTOR, T1, r0, [A, B].sort(), await membershipSnapshot(), "Equipa 1", "#16A34A", true, null, [A]]);
      const r1 = (await pool.query("SELECT revision FROM public.teams WHERE id=$1", [T1])).rows[0].revision;
      await app.query(
        "SELECT * FROM public.archive_team_atomic($1,$2,$3,$4,$5)",
        [EMP, GESTOR, T1, r1, await membershipSnapshot()]);
    } finally {
      await app.end();
    }
  }, 60_000);
