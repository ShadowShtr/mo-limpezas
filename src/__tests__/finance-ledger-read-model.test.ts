...
describe("finance unified read model", () => {
  ...
  it("UNI05: um movimento ligado não soma com a sua obrigação", () => {
    const linked = cashflow({ reference_type: "fixed_variable_payment", reference_id: "payment-1" });
    const rows = buildFinanceLedger({ payments: [payment()], cashflows: [linked] });
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ row_kind: "payment", is_linked: true, cashflow_id: "cashflow-1" });
    expect(rows.reduce((sum, row) => sum + (row.amount_cents ?? 0), 0)).toBe(15_000);
  });

  it("🔴 UNI05b: dois movimentos no mesmo pagamento não fazem um desaparecer", () => {
    // Este teste exigia `toHaveLength(1)` e dava-se por satisfeito. Com dois
    // movimentos ligados ao mesmo pagamento, o segundo era escolhido contra o
    // primeiro e **saía do razão** — 150,00 € fora do ecrã, sem aviso. Um
    // teste que exige que uma linha desapareça não protege nada: consagra a
    // perda.
    //
    // O estado é impossível na base — `cash_flow_entries_reference_unique`
    // impede-o dentro de uma empresa, e produção tem zero duplicados hoje.
    // Mas o modelo de leitura não pode depender disso para ser honesto: se
    // alguma vez chegar, mostra-se e marca-se.
    const linked = cashflow({ reference_type: "fixed_variable_payment", reference_id: "payment-1" });
    ...
    // Nada desaparece: a obrigação e o movimento a mais estão os dois lá.
    expect(rows).toHaveLength(2);
    expect(rows.map((r) => r.row_id).sort())
      .toEqual(["cashflow:cashflow-duplicate", "payment:payment-1"]);

    // E ambos ficam marcados, para ninguém ler o total como se fosse normal.
    for (const row of rows) expect(row.integrity_issue).toBe("duplicate_payment_link");

    // A obrigação continua a absorver **um** movimento — não soma os dois.
    const obrigacao = rows.find((r) => r.row_kind === "payment")!;
    expect(obrigacao.amount_cents).toBe(15_000);
    expect(obrigacao.cashflow_id).toBe("cashflow-1");
  });

  it("UNI05c: o representante é determinístico, não o que vier primeiro na lista", () => {
    const antigo = cashflow({
      id: "cashflow-antigo", reference_type: "fixed_variable_payment",
      reference_id: "payment-1", created_at: "2026-08-01T10:00:00Z",
    });
    const recente = cashflow({
      id: "cashflow-recente", reference_type: "fixed_variable_payment",
      reference_id: "payment-1", created_at: "2026-08-22T10:00:00Z",
    });
    const a = buildFinanceLedger({ payments: [payment()], cashflows: [antigo, recente] });
    const b = buildFinanceLedger({ payments: [payment()], cashflows: [recente, antigo] });
    const id = (rows: typeof a) => rows.find((r) => r.row_kind === "payment")!.cashflow_id;
    expect(id(a)).toBe("cashflow-antigo");
    expect(id(b)).toBe("cashflow-antigo");
  });
