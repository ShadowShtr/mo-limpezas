...
function construirRollback(manifesto) {
  return {
    ...
    passos: manifesto.linhas.map((l) => ({
      ...
      //
      // 🔴 A forma **completa**, não o esqueleto económico. A guarda da
      //    reversão compara descrição, notas, categoria e competência do
      //    pagamento, e descrição, data, categorias, notas e valor do
      //    movimento — e comparava-os contra campos que este manifesto não
      //    guardava. `String(undefined ?? null)` dá `"null"`, nunca igual ao
      //    valor real: a reversão recusava **sempre**, com uma queixa de
      //    alteração que nunca tinha acontecido. Estava inutilizável, e só se
      //    viu quando o ensaio da cadeia passou a montar tudo e a chegar aqui.
      //
      //    Cada campo que a guarda compara tem de nascer aqui. Acrescentar um
      //    lado sem o outro é o defeito, não a correcção.
      esperado_antes_da_reversao: {
        pagamento: {
          status: "pendente",
          amount: l.target.amount,
          due_date: null,
          description: l.target.description ?? null,
          notes: l.target.notes ?? null,
          expense_category_id: l.target.expense_category_id ?? null,
          period_year: l.target.period_year ?? null,
          period_month: l.target.period_month ?? null,
        },
        movimento: {
          status: "pendente",
          reference_id: l.target_payment_id,
          description: l.before.description ?? null,
          date: l.before.date ?? null,
          category: l.before.category ?? null,
          expense_category_id: l.before.expense_category_id ?? null,
          notes: l.before.notes ?? null,
          amount: l.before.amount ?? null,
        },
      },
    })),
  };
...
async function reverter(cliente, rollbackManifesto) {
  ...
  const plano = planoRollback(
    { linhas: rollbackManifesto.passos.map((p) => ({
        ...
        // 🔴 `repor` fica por cima: são os valores a **escrever** de volta
        //    (reference_type/reference_id), e são os que a reversão usa. O
        //    resto é o prestate do movimento, que a guarda compara.
        before: { ...p.esperado_antes_da_reversao.movimento, ...p.repor },
      })) },
