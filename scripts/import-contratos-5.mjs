...
//   node scripts/import-contratos-5.mjs --project-ref <ref> --company-id <uuid>
//   node scripts/import-contratos-5.mjs --project-ref <ref> --company-id <uuid> --apply
//
// Guardas comuns em scripts/lib/admin-db.mjs (T17-B2). A flag `--dry` antiga
// desapareceu: dry-run passou a ser o comportamento por omissão, e escrever
// passou a exigir `--apply`. O contrário — escrever por omissão, simular só se
// pedido — é a configuração que torna um engano irreversível.
import { openAdminDb } from "./lib/admin-db.mjs";

const db = await openAdminDb({
  script: "import-contratos-5.mjs",
  purpose: "importar 5 contratos recorrentes e 12 intervenções do export PARTE_1",
  writes: true,
});
const sb = db.sb;
const C = db.companyId;
...
const { data: existing, error: existingError } = await sb.from("services")
  .select("reference_number").eq("company_id", C).in("reference_number", refs);
// Sem isto, uma consulta falhada dava `taken` vazio e o script reinseria todas
// as intervenções — a idempotência dependia de a leitura ter corrido bem.
if (existingError) throw new Error(`services: ${existingError.message}`);
...
for (const [svc,c] of Object.entries(CONTRACTS)) {
  ...
  const { data: ex, error: exError } = await sb.from("contracts").select("id").eq("company_id",C).like("notes",`%${marker}%`).limit(1);
  if (exError) throw new Error(`contracts (svc${svc}): ${exError.message}`);
  ...
  const r = await db.write(
    "contracts",
    (t) => t.insert(row).select("id").single(),
    `contrato svc${svc} (${c.name})`,
  );
  if (!r.ok) process.exit(1);
  if (r.dryRun) continue;               // sem id real, as intervenções também não escrevem
  contractIdBySvc[svc] = r.data.id;
  console.log(`contrato svc${svc} criado: ${r.data.id}`);
}
...
for (const [svc,ref,date,ini,fim,estado,equipa,valor,rate] of INTERV) {
  ...
  const r = await db.write("services", (t) => t.insert(row), `intervenção ${ref}`);
  if (!r.ok) process.exit(1);
  ins++;
}
console.log(`\nContratos: ${Object.keys(contractIdBySvc).length} | intervenções: ${ins}`);
db.summary();
