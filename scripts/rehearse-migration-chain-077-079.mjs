...
// 🔴 Tudo do disco, nada de branches remotas.
//
//    A primeira versão lia a 077 e a 079 por `git show origin/<branch>:...`.
//    Funcionava enquanto o ensaio vivia numa cabeça de integração, e escondia
//    uma dependência entre branches dentro de um ficheiro que vai numa PR
//    mergeável: o resultado passava a depender do que estivesse noutro sítio.
const ler = (rel) => fs.readFileSync(path.join(REPO, rel), "utf8");
const existe = (rel) => fs.existsSync(path.join(REPO, rel));

const SQL077 = ler("supabase/migrations/077_secure_migrations_ledger.sql");
const SQL078 = ler("supabase/migrations/078_domain_mutation_change_event_foundation.sql");
const SQL073 = ler("supabase/migrations/073_payment_to_cashflow.sql");

// A 079 pode ainda não estar nesta branch — a sequência é 077 → 078 → 079, e
// cada uma chega na sua PR. Sem ela, prova-se o que há; com ela, prova-se a
// cadeia inteira. O que não se faz é ir buscá-la a outro lado para o número
// parecer maior.
const TEM_079 = existe("supabase/migrations/079_reuse_pending_cashflow_on_payment.sql");
const SQL079 = TEM_079 ? ler("supabase/migrations/079_reuse_pending_cashflow_on_payment.sql") : null;
...
try {
  ...
  v("  que revalida o valor depois do conflito", t79.rows[0].valida);
    ...
    console.log("\n── 6. 079 ainda não está nesta branch — a cadeia prova-se até à 078");
  }
  ...
  if (TEM_079) {
    ...
    v("a 079 é CREATE OR REPLACE puro — repetir não duplica", rep.rows[0].n === 2, `${rep.rows[0].n}`);
  }
} catch (e) {
