-- ============================================================================
-- MIGRATION 087 — Equipas R4: três conceitos separados, e um save por transação
-- ============================================================================
--
-- MIGRATION_NUMBER = 087. Confirmar ledger fresco antes de aplicar.
--
  ...
  SELECT to_regclass('public._migrations') IS NOT NULL
     AND EXISTS (
       SELECT 1
        ...
        WHERE name = '086_manual_charges_and_atomic_billing.sql'
     )
    INTO v_ledger_086;

  IF NOT coalesce(v_ledger_086, false) THEN
    RAISE EXCEPTION
      'EQUIPAS_R4_MISSING_086_LEDGER: 086_manual_charges_and_atomic_billing.sql ausente do ledger';
  END IF;
  ...
  PERFORM 1
    ...
    JOIN (
      SELECT DISTINCT team_id
        FROM jsonb_to_recordset(coalesce(p_vehicles, '[]'::jsonb))
          AS x(team_id uuid, vehicle_id uuid, driver_id uuid)
       WHERE team_id IS NOT NULL
    ) a ON a.team_id = t.id
...
BEGIN
  ...
  IF p_expected_revision IS DISTINCT FROM v_team.revision THEN
    RAISE EXCEPTION 'TEAM_ARCHIVE_CONFLICT'
      USING ERRCODE = 'serialization_failure',
            HINT = 'Esta equipa foi alterada por outra pessoa. Atualize para rever antes de arquivar.';
  END IF;
  ...
  IF p_expected_membership_snapshot IS DISTINCT FROM v_membership_snapshot THEN
    RAISE EXCEPTION 'TEAM_ARCHIVE_CONFLICT'
      USING ERRCODE = 'serialization_failure',
            HINT = 'As pertenças permanentes foram alteradas por outra pessoa. Atualize para rever antes de arquivar.';
  END IF;
...
REVOKE ALL PRIVILEGES ON FUNCTION public.archive_team_atomic(uuid, uuid, uuid, integer, text) FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.archive_team_atomic(uuid, uuid, uuid, integer, text) TO service_role;
...
GRANT  EXECUTE ON FUNCTION public._team_config_lock_shared(uuid) TO service_role;
REVOKE ALL PRIVILEGES ON FUNCTION public._team_config_lock_exclusive(uuid) FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public._team_config_lock_exclusive(uuid) TO service_role;
REVOKE ALL PRIVILEGES ON FUNCTION public._team_day_lock_exclusive(uuid, date) FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public._team_day_lock_exclusive(uuid, date) TO service_role;
