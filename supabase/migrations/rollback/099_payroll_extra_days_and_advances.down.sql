-- ============================================================================
-- ROLLBACK 099 — dias extras, hora extra ao valor e adiantamentos
-- ============================================================================
--
-- 🔴 APAGA DADOS que só existem aqui: os dias extras lançados, os valores por
--    dia, os adiantamentos e os valores por hora extra. O `net_salary` fica
--    com o número que tinha, mas deixa de se saber de que parcelas veio.
--
--    Se o problema for a lógica e não as colunas, a saída certa é uma
--    migration nova por cima.
--
-- As funções voltam à forma da 098: sem os campos novos, e sem as cláusulas
-- que protegiam dias extras e adiantamentos do recálculo mensal.
-- ============================================================================
...
-- ─── Colunas — a perda de dados acontece aqui ───────────────────────────────
...
ALTER TABLE public.payroll_records
  ...
  DROP COLUMN IF EXISTS extra_days_bonus,
  DROP COLUMN IF EXISTS extra_day_rate,
  DROP COLUMN IF EXISTS extra_days,
  DROP COLUMN IF EXISTS overtime_hour_rate;

ALTER TABLE public.company_settings
  DROP COLUMN IF EXISTS default_extra_day_rate,
  DROP COLUMN IF EXISTS default_overtime_hour_rate;
