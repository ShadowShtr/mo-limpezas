...
BEGIN
  ...
  IF p_patch ? 'amount' THEN
    ...
    -- `NULL` é legítimo: um pendente pode não ter valor enquanto a factura não
    -- chega. Só negativo é inválido.
...
REVOKE EXECUTE ON FUNCTION public.update_payment_atomic(uuid, uuid, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.update_payment_atomic(uuid, uuid, jsonb) FROM anon;
REVOKE EXECUTE ON FUNCTION public.update_payment_atomic(uuid, uuid, jsonb) FROM authenticated;
