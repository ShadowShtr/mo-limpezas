...
-- ---------------------------------------------------------------------------
--
-- Mesma regra da 102, e pela mesma razão: uma tabela existir não prova quem a
-- criou. Com `CREATE TABLE IF NOT EXISTS`, uma `crm_quotes` nascida fora desta
...
-- do repositório, e batem com o que `checksumForNewMigration` calcula. O par
-- LF/CRLF está pinado porque o ledger deste projecto mistura as duas
-- representações por história: o que se apanha é CONTEÚDO diferente, não fim
-- de linha diferente.
--
-- 🔴 Aplicar SÓ pelo runner canónico. O portão não impede o SQL Editor de
--    correr este ficheiro — a cadeia está no ledger, a tabela é criada, e o
--    Editor não escreve a linha. O resultado é exactamente
--    `EFFECT_WITHOUT_LEDGER`, criado por quem estava a tentar ser cuidadoso.
-- ============================================================================
...
BEGIN
  IF to_regclass('public._migrations') IS NULL THEN
    RAISE EXCEPTION
      'CRM_QUOTES_103_LEDGER_AUSENTE: public._migrations não existe — a 103 só corre pelo runner canónico';
  END IF;

  v_ledger := EXISTS (SELECT 1 FROM public._migrations WHERE name = '103_crm_orcamentos.sql');

  -- 🔴 EFEITO PARCIAL conta como efeito.
  --
  --    Olhar só para `crm_quotes` seria ingénuo: uma aplicação interrompida a
  --    meio, ou um rollback manual que apagou a tabela mas esqueceu as RPC,
  --    deixa metade da 103 de pé. Se o portão só perguntasse pela tabela,
  --    adoptava esse meio-estado e tentava construir por cima — e o
  --    `CREATE FUNCTION OR REPLACE` por cima de uma RPC de outra versão é
  --    exactamente o tipo de coisa que não dá erro e fica errada.
  --
  --    Qualquer vestígio exclusivo da 103 basta para parar.
  SELECT array_agg(efeito.nome ORDER BY efeito.nome) INTO v_efeitos
    FROM (VALUES
      ('tabela crm_quotes',            to_regclass('public.crm_quotes')::text),
      ('tabela crm_quote_items',       to_regclass('public.crm_quote_items')::text),
      ('índice uq_crm_quotes_numero',  to_regclass('public.uq_crm_quotes_numero')::text),
      ('índice uq_crm_quotes_seq_revisao',  to_regclass('public.uq_crm_quotes_seq_revisao')::text),
      ('índice uq_crm_quotes_revisao_viva', to_regclass('public.uq_crm_quotes_revisao_viva')::text),
      ('índice uq_crm_quote_items_posicao', to_regclass('public.uq_crm_quote_items_posicao')::text),
      ('índice crm_quotes_id_company_unique', to_regclass('public.crm_quotes_id_company_unique')::text),
      ('RPC create_crm_quote_with_items',
       to_regprocedure('public.create_crm_quote_with_items(uuid, uuid, uuid, uuid, text, integer, date, date, text, numeric, boolean, numeric, text, jsonb, text, text, text, uuid, jsonb)')::text),
      ('RPC revise_crm_quote',
       to_regprocedure('public.revise_crm_quote(uuid, uuid, uuid, date, date, numeric, boolean, numeric, text, jsonb)')::text),
  ...
  SELECT array_agg(requisito.nome ORDER BY requisito.nome) INTO v_faltam
   ...
   WHERE NOT EXISTS (SELECT 1 FROM public._migrations m WHERE m.name = requisito.nome);

  IF v_faltam IS NOT NULL THEN
    RAISE EXCEPTION
      'CRM_QUOTES_103_MISSING_PREREQUISITE_LEDGER: sem linha de ledger para % — a cadeia 101→101a→101b→102 não está provada',
      v_faltam;
  END IF;

  -- Um checksum NULL cai aqui de propósito: não saber é indistinguível de
  -- estar errado.
  SELECT array_agg(requisito.nome ORDER BY requisito.nome) INTO v_erradas
    FROM (VALUES
      ...
      ('102_crm_visitas_comerciais.sql',
       ...
       '781f88bd47102bc13bf68767a9c6639e543b2a816c768e26e39021b27fc7f01f')
    ) AS requisito(nome, lf, crlf)
   WHERE NOT EXISTS (
     SELECT 1 FROM public._migrations m
      WHERE m.name = requisito.nome AND m.checksum IN (requisito.lf, requisito.crlf)
   );

  IF v_erradas IS NOT NULL THEN
    RAISE EXCEPTION
      'CRM_QUOTES_103_CHECKSUM_MISMATCH_PREREQUISITE: o ledger tem outro conteúdo para % — a cadeia sob a 103 não é a canónica',
      v_erradas;
  END IF;
END
$proveniencia$;
...
-- 🔴 Sem `IF NOT EXISTS`: o portão acima já provou que não existe. Se
--    existisse aqui, algo correu entre as duas e o certo é rebentar.
...
-- 🔴 Aqui havia um `ADD COLUMN IF NOT EXISTS source_lead_id` e um `UPDATE` de
--    backfill, para o caso de uma base onde a 103 já tivesse corrido numa
--    versão anterior.
--
--    Os dois saíram, e por uma razão que vale a pena escrever: com o portão de
--    proveniência acima, esta migration só corre quando `crm_quotes` NÃO
--    existe. A tabela é sempre criada de raiz, já com `source_lead_id` na
--    definição, e está vazia quando se chega aqui. O `ADD COLUMN` nunca tinha
--    nada que acrescentar e o `UPDATE` nunca tinha uma linha para tocar.
--
--    Código morto numa migration não é inofensivo: era ele que justificava a
--    excepção no trigger de imutabilidade logo abaixo — e essa excepção
--    deixava passar uma escrita que não devia passar.
--
--    Produção nunca teve uma 103. Não há nada a adoptar.
...
AS $imutavel$
BEGIN
  -- 🔴 QUALQUER mudança é recusada. Não há excepção para «preencher um valor
  --    em branco».
  --
  --    A versão anterior só protegia `OLD.source_lead_id IS NOT NULL`, para
  --    deixar passar o backfill de uma 103 antiga. Consequência: `NULL → valor`
  --    passava — e um caminho de escrita futuro podia atribuir a um orçamento
  --    uma lead de origem que ele nunca teve, meses depois, sem rasto. Um
  --    palpite gravado é pior do que um `NULL` que diz «não se sabe».
  --
  --    O backfill saiu (a tabela nasce de raiz, vazia), por isso a excepção
  --    deixou de ter sequer o motivo que a justificava.
  --
  --      valor → outro   REJECT        NULL → valor   REJECT
  --      valor → NULL    REJECT        NULL → NULL    passa (não é mudança)
