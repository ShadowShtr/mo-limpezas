-- ============================================================================
-- 091 — recorrência explícita dos pagamentos fixos
-- ============================================================================
--
...
DO $dependencia$
BEGIN
  -- Esta migration assenta no protocolo de periodo da 089. Sem ele, a
  -- preparacao voltaria a decidir "aberto" fora da transacao que escreve.
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public' AND p.proname = 'assert_financial_period_open_locked'
  ) THEN
    RAISE EXCEPTION 'RECURRENCE_091_REQUIRES_089: protocolo de periodo financeiro nao instalado.';
  END IF;
END
$dependencia$;
...
BEGIN
  ...
  -- O lock e a decisao sobre o periodo passam a ser a MESMA operacao, vinda do
  -- protocolo partilhado da 089. Antes, quem chamava esta RPC verificava o
  -- periodo em TypeScript, noutra viagem: entre a verificacao e a
  -- materializacao cabia um fecho, e o mes fechava com linhas a entrar.
  --
  -- A chave e a mesma que o fecho usa - hashtext(empresa) + ano*100+mes -, e e
  -- por isso que os dois lados se veem. Um lock proprio aqui protegeria o duplo
  -- clique e continuaria cego ao closeFinancialPeriod.
  PERFORM public.assert_financial_period_open_locked(p_company_id, p_year, p_month);
...
-- ---------------------------------------------------------------------------
-- Configurar a periodicidade, sob o mesmo protocolo
-- ---------------------------------------------------------------------------
--
-- A configuracao vivia em TypeScript: ler o pagamento, verificar o periodo,
-- e depois um UPDATE separado. Tres viagens, e entre a segunda e a terceira
-- cabia um fecho de mes. Aqui e uma transacao so, com o lock partilhado.
--
-- O periodo que manda e o da COMPETENCIA do pagamento, nao o de hoje: mexer na
-- periodicidade de uma obrigacao de Julho e mexer em Julho.
CREATE OR REPLACE FUNCTION public.configure_payment_recurrence_atomic(
  p_company_id uuid,
  p_payment_id uuid,
...
BEGIN
  IF p_interval_months IS NULL OR p_interval_months <= 0 THEN
    RAISE EXCEPTION 'RECURRENCE_INVALID_INTERVAL: %', p_interval_months;
  END IF;
  IF p_anchor_date IS NULL THEN
    RAISE EXCEPTION 'RECURRENCE_INVALID_ANCHOR';
  END IF;

  -- Ler com o lock de linha: a competencia nao pode mudar debaixo dos pes
  -- entre saber qual e o mes e bloquea-lo.
  SELECT kind, recurring, period_year, period_month
    INTO v_kind, v_recurring, v_year, v_month
    FROM public.fixed_variable_payments
   WHERE id = p_payment_id AND company_id = p_company_id
     FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'RECURRENCE_PAYMENT_NOT_FOUND';
  END IF;
  IF v_kind <> 'fixo' OR v_recurring IS NOT TRUE THEN
    RAISE EXCEPTION 'RECURRENCE_ONLY_FIXED_RECURRING';
  END IF;

  PERFORM public.assert_financial_period_open_locked(p_company_id, v_year, v_month);
  ...
  UPDATE public.fixed_variable_payments
     SET recurrence_interval_months = p_interval_months,
         recurrence_anchor_date = p_anchor_date
   WHERE id = p_payment_id AND company_id = p_company_id;
END;
$fn$;

REVOKE ALL PRIVILEGES ON FUNCTION public.configure_payment_recurrence_atomic(uuid, uuid, integer, date, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.configure_payment_recurrence_atomic(uuid, uuid, integer, date, uuid)
  TO service_role;
