-- ============================================================================
-- 090 — publicar manual_charges no Realtime
-- ============================================================================
--
...
BEGIN
  ...
  IF NOT coalesce(v_tabela, false) THEN
    RAISE EXCEPTION 'REALTIME_090_PRECONDITION_FAILED: public.manual_charges não existe (a 086 não correu?)';
  END IF;
  IF NOT coalesce(v_pub, false) THEN
    RAISE EXCEPTION 'REALTIME_090_PRECONDITION_FAILED: a publicação supabase_realtime não existe';
  END IF;
END
$precondicoes$;
...
BEGIN
  ...
  IF NOT v_publicada THEN
    RAISE EXCEPTION 'REALTIME_090_POSTSTATE_FAILED: manual_charges não ficou na publicação';
  END IF;
  IF v_identidade <> 'f' THEN
    RAISE EXCEPTION 'REALTIME_090_POSTSTATE_FAILED: REPLICA IDENTITY é %, esperado FULL', v_identidade;
  END IF;
END
$posestado$;
